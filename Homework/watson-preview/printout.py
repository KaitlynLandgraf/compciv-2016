...
htmlfile.write("<h1>Hi Dan this is Kaitlyn and here are some photos of various Georges.</h1>")
...
for jsonname in glob(join(RECOG_DIR, '*.json')):
    ...
    htmlfile.write('<ol>')
    for x in range(len(j['images'][0]['scores'])):
        namewrite = j['images'][0]['scores'][x]['name']
        scorewrite = j['images'][0]['scores'][x]['score']
        htmlfile.write("<li> %s -- %s </li>" %(namewrite, scorewrite))
    htmlfile.write('</ol>')
