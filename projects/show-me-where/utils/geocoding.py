...
def geocode(location):
  ...
  it expects 'location' is a string, representing some kind of human-readable geographical location, e.g., "Stanford, CA"

  It also expects the variable "CREDS_FILE" to point to an existing file that contains a valid Mapzen Search key.

  What it does:
  -------
  It opens and reads the file at CREDS_FILE to get the API key.

  It calls the Mapzen Search API via a HTTP request, using the API key, and the user-provided 'location' string as the 'text' parameter.
  ...
  --confidence: a foat representing the confidence value that mapzen has in its reslt.
